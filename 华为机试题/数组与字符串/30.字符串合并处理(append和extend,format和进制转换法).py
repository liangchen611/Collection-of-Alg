...
#——【进制转换法（int和format)】——int字符，format数值
# 通过int和format都可以转换格式，不过二者有所区别【二者返回结果都是字符串，int只接受字符串，format只能接收整数】
# int：N转十
# int(i, n)——其中n代表了进制，此方法的作用是【将n进制下的字符串i转换为十进制下的字符串】
# 必须接受的是字符串，其他类型的不可用，n表示转换时将i视为什么进制的数
# 如果i中出现了n所不具有的字符，那么会报错，例如int(“G0A",16),G不属于十六进制的字符范围内，所以会报错
...
# 例如，format(int(n,2),"04x")，然后转换为0填充的4位小写十六进制的字符串。